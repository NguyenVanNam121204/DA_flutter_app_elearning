...
var configuredFrontendOrigins =
    ...
    Array.Empty<string>();
var fallbackFrontendOrigins = new[]
{
    frontendUrl,
    "http://localhost:3000",
    "http://localhost:5173",
    "http://localhost:63613",
    "http://127.0.0.1:3000",
    "http://127.0.0.1:5173",
    "http://127.0.0.1:63613"
};
var allowedFrontendOriginSet = new HashSet<string>(StringComparer.OrdinalIgnoreCase);

foreach (var origin in configuredFrontendOrigins)
{
    if (!string.IsNullOrWhiteSpace(origin))
    {
        allowedFrontendOriginSet.Add(origin.Trim());
    }
}

foreach (var origin in fallbackFrontendOrigins)
{
    if (!string.IsNullOrWhiteSpace(origin))
    {
        allowedFrontendOriginSet.Add(origin.Trim());
...
builder.Services.AddCors(options =>
{
    options.AddPolicy("AllowFrontend", p =>
        p.SetIsOriginAllowed(origin =>
        {
            ...
            if (allowedFrontendOriginSet.Contains(origin))
            {
                return true;
            }
...
builder.Services.AddScoped<IQuizAttemptMapper, QuizAttemptMapperService>(); // Mapper service for quiz attempts
...
builder.Services.AddScoped<IQuizGroupMediaService, QuizGroupMediaService>();
builder.Services.AddScoped<IQuestionMediaService, QuestionMediaService>();
builder.Services.AddScoped<IAssetFrontendMediaService, AssetFrontendMediaService>();
builder.Services.AddScoped<IPronunciationMediaService, PronunciationMediaService>();
...
// Infrastructure-level helper for building public URLs (used by MediaServices)
