...
export default function CreateQuestionModal({
  ...
  const renderMCQOptions = () => (
    <div className="options-grid"><Row>{qFormData.options.map((option, index) => (
      <Col md={6} key={`option-${index}-${option.text}`} className="mb-3"><div className="option-item d-flex align-items-center gap-2">
  ...
  const renderMatchingOptions = () => (
    <><div className="alert alert-info py-2 small mb-3"><FaQuestionCircle className="me-2" />Nhập các cặp tương ứng. Backend sẽ dùng Text để đối chiếu ID.</div>
      {qFormData.matchingPairs.map((pair, index) => (
        <div key={`pair-${index}-${pair.key}-${pair.value}`} className="option-item mb-2"><Row className="align-items-center g-2">
  ...
  const renderOrderingOptions = () => (
    <><div className="alert alert-info py-2 small mb-3"><FaQuestionCircle className="me-2" />Sắp xếp theo đúng thứ tự logic.</div>
      {qFormData.options.map((option, index) => (
        <div key={`ordering-${index}-${option.text}`} className="option-item mb-2 d-flex align-items-center gap-3"><span className="badge bg-secondary rounded-circle p-2 d-flex align-items-center justify-content-center" style={{ width: '30px', height: '30px' }}>{index + 1}</span>
